...
  const [locationSearch, setLocationSearch] = useState(""); // NOVO: Estado para busca de localização
...
    // 1. Filtragem por Deficiência (somente para candidatos na view "explorar")
    if(view==="explorar" && !isEmpresa && filterDef!=="todas"){
      const v = filterDef.toLowerCase();
      list = list.filter(j=> j.deficiencia===v || j.deficiencia==="qualquer");
    }

    // 2. Filtragem por Localização (NOVO: Case-insensitive)
    if(locationSearch.trim().length > 0){
      const search = locationSearch.toLowerCase().trim();
      list = list.filter(job => job.location && job.location.toLowerCase().includes(search));
    }
...
        {/* NOVO: Barra de Busca de Localização e Filtro de Deficiência */}
        <div style={{display:"flex", gap:12, marginBottom:12, flexDirection: isMobile ? "column" : "row"}}>
          <input 
            placeholder="Buscar por Localização (ex: São Paulo, SP)"
...
            <select 
              style={{padding:10,borderRadius:999,border:`1px solid ${COLORS.border}`}} 
              value={filterDef} 
              onChange={(e)=>setFilterDef(e.target.value)}
            >
