...
export default function Profile({ user: perfilUsuarioExterno = null }) {
  ...
  useEffect(() => {
    if (!usuarioLogado && !perfilUsuarioExterno) {
      setCarregando(false);
      return;
    }

    if (perfilUsuarioExterno) {
      setPerfil(perfilUsuarioExterno);
      setCarregando(false);
    } else if (profileData) {
      setPerfil({ ...profileData, email: usuarioLogado.email });
      setCarregando(false);
    } else {
      const unsubscribe = useAuth.subscribe(
        ...
        (profile) => {
          if (profile) {
            setPerfil({ ...profile, email: usuarioLogado.email });
            setCarregando(false);
            unsubscribe();
          }
        }
  ...
  }, [usuarioLogado, profileData, perfilUsuarioExterno]);
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', paddingTop: '1rem', paddingBottom: '1rem', backgroundColor: '#ECECEC' }}>
      ...
      {!isEditing && (
        <div style={{ width: containerWidth, backgroundColor: '#ECECEC', borderRadius: '1.5rem', boxShadow: '0 10px 15px -3px rgba(0,0,0,0.1),0 4px 6px -2px rgba(0,0,0,0.05)', padding: '1.5rem', marginTop: '4rem', position: 'relative' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: '1.5rem', position: 'relative' }}>
            ...
            {isMeuPerfil && (
              <button onClick={() => setIsEditing(true)} style={{ position: 'absolute', bottom: 0, right: '50%', transform: 'translateX(50%)', width: '2rem', height: '2rem', borderRadius: '9999px', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.1),0 2px 4px -1px rgba(0,0,0,0.06)', cursor: 'pointer', backgroundColor: '#E6E69D', border: 'none' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <div style={{ padding: '0.75rem', borderRadius: '9999px', border: '1px solid #D9D9D9', backgroundColor: '#fff', color: '#000', textAlign: 'center' }}>{perfil.nome || 'Nome não informado'}</div>
            <div style={{ padding: '0.75rem', borderRadius: '9999px', border: '1px solid #D9D9D9', backgroundColor: '#fff', color: '#000', textAlign: 'center' }}>{perfil.telefone || 'Telefone não informado'}</div>
            <div style={{ padding: '0.75rem', borderRadius: '9999px', border: '1px solid #D9D9D9', backgroundColor: '#fff', color: '#000', textAlign: 'center' }}>{perfil.email || 'Email não informado'}</div>
            <div style={{ padding: '0.75rem', borderRadius: '9999px', border: '1px solid #D9D9D9', backgroundColor: '#fff', color: '#000', textAlign: 'center' }}>{perfil.dataNascimento || 'Data de nascimento não informada'}</div>
          </div>
        </div>
      )}
      ...
      {isEditing && isMeuPerfil && (
        <div style={{ width: containerWidth, backgroundColor: '#ECECEC', borderRadius: '1.5rem', boxShadow: '0 10px 15px -3px rgba(0,0,0,0.1),0 4px 6px -2px rgba(0,0,0,0.05)', padding: '1.5rem', marginTop: '4rem', position: 'relative' }}>
          <button onClick={() => setIsEditing(false)} style={{ position: 'absolute', left: '1rem', top: '1rem', width: '2.5rem', height: '2.5rem', borderRadius: '9999px', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: '#D9D9D9', border: 'none', cursor: 'pointer' }}>
            ←
          </button>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <input type="text" name="nome" placeholder="Nome" value={perfil.nome} onChange={handleChange} style={{ width: '100%', padding: '0.75rem', borderRadius: '9999px', border: '1px solid #D9D9D9', outline: 'none', backgroundColor: '#fff', color: '#000' }} disabled={uploading} />
            <input type="tel" name="telefone" placeholder="Telefone" value={perfil.telefone} onChange={handleChange} style={{ width: '100%', padding: '0.75rem', borderRadius: '9999px', border: '1px solid #D9D9D9', outline: 'none', backgroundColor: '#fff', color: '#000' }} disabled={uploading} />
            <input type="email" name="email" placeholder="Email" value={perfil.email} disabled style={{ width: '100%', padding: '0.75rem', borderRadius: '9999px', border: '1px solid #D9D9D9', backgroundColor: '#fff', color: '#000' }} />
            <input type="text" name="dataNascimento" placeholder="Data de nascimento" value={perfil.dataNascimento} onChange={handleChange} style={{ width: '100%', padding: '0.75rem', borderRadius: '9999px', border: '1px solid #D9D9D9', outline: 'none', backgroundColor: '#fff', color: '#000' }} disabled={uploading} />

            <button type="button" onClick={handleSave} style={{ width: '100%', padding: '0.75rem', borderRadius: '9999px', fontWeight: 'bold', backgroundColor: '#E6E69D', color: '#000', border: 'none', cursor: 'pointer', marginTop: '1rem' }} disabled={uploading}>
