...
import useAuthStore from '../store/useAuth'; // Usando useAuthStore para ter acesso ao estado completo

export default function Jobs() {
  // Importamos user e profileData para garantir acesso ao UID e tipo
  const { user, profileData } = useAuthStore(); 
  const isEmpresa = profileData?.tipoUsuario === 'empresa';
  const isCandidato = profileData?.tipoUsuario === 'pcd'; // Alterado para 'pcd' como no seu Profile.jsx
...
  // FUNÇÃO DE CANDIDATURA (COM NOTIFICAÇÃO EM TEMPO REAL)
  const handleApply = async (job) => {
    // Verifica se é candidato e se os dados estão carregados
    if (!user || profileData?.tipoUsuario !== 'pcd') {
        alert("Você precisa estar logado como candidato (PCD) para se candidatar!");
        return;
    }

    const isApplied = appliedJobs.some((j) => j.id === job.id);

    // 1. CANCELAR CANDIDATURA
    if (isApplied) {
...
    // 3. CRIA A NOTIFICAÇÃO PARA A EMPRESA
    if (job.empresaUid) {
        try {
...
                jobTitle: job.title,
                createdAt: serverTimestamp(), // ESSENCIAL para tempo real e ordenação
                read: false,
            });
        } catch (error) {
...
  // FUNÇÃO PARA ADICIONAR VAGA (SALVANDO O UID DA EMPRESA)
...
        deficiencia: newJob.deficiencia,
        empresaUid: user.uid, // ESSENCIAL para o sistema de notificação funcionar
...
    const listToFilter = view === "explorar" ? jobs : view === "candidatadas" ? appliedJobs : savedJobs;
    ...
    // Se for Empresa, só mostra as vagas que ela mesma criou
    if (isEmpresa && user) {
...
      return filteredList.filter(job => job.deficiencia === filterDeficiencia.toLowerCase());
    }
...
                  
                  {/* Botão de Candidatar-se SÓ para candidatos */}
                  {isCandidato && (
