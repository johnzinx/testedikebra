...
export default function Notifications() {
  ...
  const [curriculos, setCurriculos] = useState({}); // 🔹 armazena currículos por senderUid
  ...
  // 🔹 Busca as notificações e também os currículos dos remetentes
  useEffect(() => {
    if (!user || !user.uid) {
      setLoading(false);
      return;
    ...
    const unsubscribe = onSnapshot(notificationsQuery, async (snapshot) => {
      ...
      // 🔹 busca currículos dos remetentes
            ...
            <li key={n.id} style={styles.card(n.read)}>
              ...
              {n.senderUid && n.senderUid !== user.uid && (
                <div style={styles.actionButtons}>
                  ...
                  {/* 🔹 Botão currículo só para inscrição em vaga */}
                  ...
                  <button onClick={() => handleViewProfile(n)} style={styles.profileButton}>
                    <FiUser style={{ marginRight: '0.3rem' }} /> Perfil
                  </button>
                  <button onClick={() => handleChat(n)} style={styles.chatButton}>
